...
Performance: OpenAI client and FAISS index are cached as module-level
singletons so they are never re-initialised or re-loaded on each request.
"""
...
CHUNK_OVERLAP    = int(os.getenv("CHUNK_OVERLAP", "175"))
...
def get_cached_index(storage_dir: str) -> tuple[faiss.Index, list[dict]]:
    """Return FAISS index + metadata, loading from disk only on first call."""
...
def invalidate_index_cache() -> None:
    """Call after re-ingestion so the next query reloads a fresh index."""
    global _faiss_index, _metadata_cache
    _faiss_index    = None
...
def _chunk_text(text: str, chunk_size: int = CHUNK_SIZE, overlap: int = CHUNK_OVERLAP) -> list[str]:
    ...
    while start < text_len:
        ...
        # Snap to whitespace only if it keeps us well ahead of start
        if end < text_len:
...
def build_index(sections: list[Section], storage_dir: str) -> int:
    ...
    for section in sections:
        for idx, chunk in enumerate(_chunk_text(section.full_text)):
    ...
    # Update in-memory cache immediately so next query uses fresh index
...
def similarity_search(query: str, storage_dir: str, top_k: int = 6) -> list[tuple[dict, float]]:
    """
    Embed *query* and search the cached FAISS index.
    Returns list of (metadata_dict, cosine_score) sorted by descending score.
    """
    client        = get_openai_client()
